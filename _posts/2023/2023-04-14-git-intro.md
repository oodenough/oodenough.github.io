...
![git-workflow](/postimgs/2023/git-workarea.png)

分别是 `工作区` 、 `暂存区` 和 `仓库`.

当前对代码的修改是在工作区进行的，而代码最终会用 `git commit` 命令提交到仓库，暂存区顾名思义是将本地修改后的代码提交到仓库的一个过渡区。

如果当前工作区的代码出现问题，可以从暂存区和仓库的 `commit` 拉取代码来覆盖当前的工作区。

所以接下来分代码提交和代码拉取来介绍 git 的几条常用命令。
...
这里使用的 URL 是 ssh，原因前面讲过了：在对私有仓库执行相关操作时需要 ssh 密钥对验证身份
...
![good](https://media.giphy.com/media/mCIjCgs3nWQWfJZvPA/giphy.gif)